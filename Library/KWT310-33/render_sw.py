...
Top-down-ish isometric so the bowl interior and ledge are visible."""
import os, math, glob
import numpy as np
...
STEEL = np.array([192, 196, 202], float)
...
for o in doc.Objects:
    ...
    for f in fs:
        tris.append((P[list(f)], STEEL))


def n(v):
    return v / np.linalg.norm(v)

view = n(np.array([-0.38, 0.60, -1.0]))   # front-above, looking down into the bowl
up = np.array([0, 0, 1.0])
