...
Front-above iso so the tank, bowl and open seat read.  -> preview_iso.png
White porcelain; the flush lever is chrome."""
import os, math, glob
import numpy as np
...
WHITE = np.array([238, 238, 234], float)
CHROME = np.array([140, 145, 150], float)


def colour_for(name):
    return CHROME if name.startswith(("Lever", "Drain", "Overflow")) else WHITE
...
view = n(np.array([-0.35, 0.55, -1.0]))   # from front-above, looking down into the well
up = np.array([0, 0, 1.0])
