"""Software render (no OpenGL).  freecadcmd render_sw.py  -> preview_iso.png
Front-above iso (front face is +Y)."""
import os, math, glob
import numpy as np
...
WHITE = np.array([243, 243, 237], float)
DARK = np.array([56, 56, 64], float)
METAL = np.array([150, 152, 156], float)
...
for o in doc.Objects:
    ...
    if sh is None or sh.isNull() or not o.TypeId.startswith("Part::"):
        continue
    col = colour_for(o.Name)
    vs, fs = sh.tessellate(0.3)
...
view = n(np.array([0.45, -0.78, -0.30]))   # FRONT-right, slightly above
up = np.array([0, 0, 1.0])
...
for p, col in proj:
    ...
    if nz[2] < 0:
        nz = -nz
    inten = min(1.0, 0.34 + 0.70 * max(0.12, nz[2]))
