...
for o in doc.Objects:
    ...
    if sh is None:
        continue
    nm = o.Name.lower()
    col = DARK if any(k in nm for k in DARK_PARTS) else STEEL
...
view = n(np.array([-0.50, -1.0, -0.62]))   # front-right, slightly high
up = np.array([0, 0, 1.0])
...
for p, col in proj:
    ...
    if nz[2] < 0:
        nz = -nz
    inten = min(1.0, 0.30 + 0.75 * max(0.14, nz[2]))
