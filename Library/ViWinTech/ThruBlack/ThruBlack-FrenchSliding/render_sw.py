"""Software render of every ThruBlack size variant under this folder (no OpenGL).
freecadcmd render_sw.py  ->  writes preview_iso.png next to each .FCStd.
Vinyl reads ThruBlack; glass reads light blue-grey; viewed from the exterior."""
import os, math, glob
import numpy as np
...
BLACK = np.array([28, 28, 31], float)     # ThruBlack vinyl
GLASS = np.array([139, 168, 186], float)  # glazing
...
def render(fc):
    ...
    for o in doc.Objects:
        ...
        if sh is None:
            continue
        col = GLASS if "glass" in o.Name.lower() else BLACK
        vs, fs = sh.tessellate(1.0)
        P = np.array([[v.x, v.y, v.z] for v in vs], float)
        for f in fs:
            tris.append((P[list(f)], col))

    # Camera on the exterior (-Y) side, slightly above and to the left.
    view = n(np.array([-0.45, 1.0, -0.38]))
    up = np.array([0, 0, 1.0])
    ...
    for p, col in proj:
        ...
        if nz[2] < 0:
            nz = -nz
        inten = min(1.0, 0.32 + 0.72 * max(0.15, nz[2]))
    ...
    Image.fromarray(img.astype(np.uint8)).save(
        os.path.join(os.path.dirname(fc), "preview_iso.png"))
...
for fc in sorted(glob.glob(os.path.join(OUT, "**", "*.FCStd"), recursive=True)):
    t = render(fc)
    total += t
    print("  rendered %s (%d tris)" % (os.path.basename(fc), t))
print("SW_RENDER_DONE", total)
