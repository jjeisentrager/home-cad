...
WHITE = np.array([244, 244, 247], float)
LIGHT = np.array([205, 209, 214], float)
DARK = np.array([45, 45, 50], float)
GLASS = np.array([28, 32, 40], float)


def colour(nm):
    if "glass" in nm:
        return GLASS
    if "display" in nm or "dial" in nm:
        return DARK
    if "bezel" in nm:
        return LIGHT
    return WHITE
