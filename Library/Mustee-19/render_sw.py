...
Front-above iso so the open basin, legs and brace frame read.  -> preview_iso.png
White tub, gray steel legs/braces, dark feet + drain."""
import os, math, glob
import numpy as np
...
WHITE = np.array([238, 238, 233], float)
STEEL = np.array([158, 163, 168], float)
DARK = np.array([52, 54, 58], float)
...
view = n(np.array([0.50, -0.66, -0.40]))   # front-right, above -> see into the basin
up = np.array([0, 0, 1.0])
