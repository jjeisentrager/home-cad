...
STEEL = np.array([190, 194, 200], float)
DARK = np.array([35, 35, 40], float)
...
for o in doc.Objects:
    ...
    if sh is None:
        continue
    col = DARK if ("handle" in o.Name.lower() or "dispenser" in o.Name.lower()) else STEEL
    vs, fs = sh.tessellate(2.0)
    P = np.array([[v.x, v.y, v.z] for v in vs], float)
    for f in fs:
        tris.append((P[list(f)], col))

# ---- camera (front-right-top isometric) -----------------------------------
def n(v):
    return v / np.linalg.norm(v)

view = n(np.array([-0.55, -1.0, -0.42]))      # camera -> scene
up = np.array([0, 0, 1.0])
right = n(np.cross(view, up))
tup = n(np.cross(right, view))
light = n(np.array([-0.3, -0.7, 0.65]))       # from upper-front-left
...
for p, col in proj:
    ...
    a, b, c = p
    # shading uses screen-space normal sign via depth cross
    e1, e2 = b - a, c - a
    nz = np.cross(e1, e2)
    ...
    if nn == 0:
        continue
    nz = nz / nn
    # orient toward camera (camera looks along +view-depth = +z in proj space)
    if nz[2] < 0:
        nz = -nz
    inten = 0.32 + 0.68 * max(0.0, nz[2]) * 0.6 + 0.4 * max(0.0, nz[2])
...
# flip horizontally so it reads naturally (dispenser door on the left, as faced)
img = img[:, ::-1]
