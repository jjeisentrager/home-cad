"""Software render of the framing (no OpenGL). freecadcmd render_sw.py
Front-above isometric so the stud spacing and plates read clearly."""
import os, math
import numpy as np
...
doc = App.openDocument(os.path.join(OUT, "MainFraming.FCStd"))

WOOD = np.array([209, 173, 115], float)
...
for o in doc.Objects:
    ...
    if sh is None:
        continue
    vs, fs = sh.tessellate(1.5)
    P = np.array([[v.x, v.y, v.z] for v in vs], float)
    for f in fs:
        tris.append((P[list(f)], WOOD))


def n(v):
    return v / np.linalg.norm(v)

view = n(np.array([-0.55, -0.7, -0.45]))   # front-above looking down at the walls
up = np.array([0, 0, 1.0])
