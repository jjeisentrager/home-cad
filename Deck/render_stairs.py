"""Software render of the deck viewed from the SE corner + above, so both new
stair flights read. freecadcmd render_stairs.py -> stairs_iso.png"""
import os, math
import numpy as np
...
doc = App.openDocument(os.path.join(OUT, "Deck.FCStd"))

COLS = {
    "Decking": (158, 112, 74), "Joists": (140, 102, 61),
    "RimAndLedger": (128, 92, 56), "Beams": (128, 92, 56),
    "Piers": (184, 184, 179), "Footings": (184, 184, 179),
    "BearingPlates": (115, 120, 128),
    "StairEast": (198, 96, 60), "StairSouth": (60, 120, 198),  # tinted to spot them
    "StairPads": (184, 184, 179),
}
DEFAULT = (170, 170, 170)
...
view = n(np.array([-0.6, -0.6, -0.45]))   # from SE + above, looking at the corner
up = np.array([0, 0, 1.0])
...
Image.fromarray(img.astype(np.uint8)).save(os.path.join(OUT, "stairs_iso.png"))
